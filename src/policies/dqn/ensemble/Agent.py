...
from src.policies.dqn.ensemble.Ensemble import MLPEnsemble
...
class Agent(nn.Module):
    ...
    def __init__(self,
        ...
        if self.encoder_architecture_name == "MLP":
            self.net = MLPEnsemble(obs_size=obs_size,
                                   n_actions=n_actions,
                                   ...
                                   seed=seed)
            self.target_net = MLPEnsemble(obs_size=obs_size,
                                          n_actions=n_actions,
                                          hidden_dims=hidden_dims,
                                          k_lipschitz=k_lipschitz,
                                          bilipschitz=bilipschitz,
    ...
    def get_action(self, env: gym.Env, epsilon: float, device: str) -> int:
        ...
        if self.exploration_strategy_name == "epsilon-greedy":
            ...
            mean_q_values, var_q_values, epistemic_var_q_values = torch.mean(means_q_values, -2), torch.mean(vars_q_values, -2), torch.var(means_q_values, -2)  # Useful to track mean, aleatoric, epistemic
    ...
    def mse_loss(self, batch: Tuple[Tensor, Tensor]) -> Tensor:
        ...
        means_state_action_values, vars_state_action_values = self.net(states, aggregate=False)  # [B, S, A]
        batch_size, n_samples, _ = means_state_action_values.shape
        means_state_action_values = means_state_action_values.gather(2, actions.view(batch_size, 1).repeat(1, n_samples).unsqueeze(-1)).squeeze(-1)
        vars_state_action_values = vars_state_action_values.gather(2, actions.view(batch_size, 1).repeat(1, n_samples).unsqueeze(-1)).squeeze(-1)

        with torch.no_grad():
            means_next_state_values, vars_next_state_values = self.target_net(next_states, aggregate=False)  # [B, S, A]
            means_next_state_values = means_next_state_values.max(-1)[0]
            means_next_state_values[dones.view(batch_size, 1).repeat(1, n_samples)] = 0.0
            means_next_state_values = means_next_state_values.detach()

        expected_state_action_values = means_next_state_values * self.gamma + rewards.view(batch_size, 1).repeat(1, n_samples)

        return nn.MSELoss()(means_state_action_values, expected_state_action_values)
    ...
    def smooth_L1_loss(self, batch: Tuple[Tensor, Tensor]) -> Tensor:
        ...
        return nn.SmoothL1Loss()(means_state_action_values, expected_state_action_values)
    ...
    def gaussian_nll_loss(self, batch: Tuple[Tensor, Tensor]) -> Tensor:
        ...
        return nn.GaussianNLLLoss()(means_state_action_values, expected_state_action_values, vars_state_action_values)
