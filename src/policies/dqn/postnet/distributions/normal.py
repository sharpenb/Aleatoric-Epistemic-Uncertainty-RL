...
class NormalGammaPrior(ConjugatePrior):
    ...
    def update(self, update: PosteriorUpdate) -> Posterior:
        ...
        # print(mu, lambd, alpha, beta, z1, z2)
...
class NormalGamma(Posterior):
    ...
    def rsample(self, n_samples=[]) -> torch.Tensor:
        tau_samples = torch.distributions.gamma.Gamma(self.alpha, self.beta).rsample(n_samples)
        normal_samples = torch.distributions.Normal(self.mu, (1/(self.lambd * tau_samples))**.5).rsample(n_samples)
        return normal_samples, 1/(self.lambd * tau_samples)**2  # return mean, variance
...
class Normal(Likelihood):
    ...
    def rsample(self, n_samples=[]) -> torch.Tensor: # TODO: check the meaning of sampling from maximum a posterior
        normal_samples = torch.distributions.Normal(self.loc, (1 / (self.precision))**.5).rsample(n_samples)  # TODO: check arguments and "1"
