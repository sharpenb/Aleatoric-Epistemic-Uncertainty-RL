...
from src.policies.dqn.postnet.postnet import MLPPostNet
...
class Agent(nn.Module):
    ...
    def __init__(self,
        ...
        if self.encoder_architecture_name == "MLP":
            self.net = MLPPostNet(obs_size=obs_size,
                                  n_actions=n_actions,
                                  ...
                                  seed=seed)
            self.target_net = MLPPostNet(obs_size=obs_size,
                                         n_actions=n_actions,
                                         hidden_dims=hidden_dims,
                                         k_lipschitz=k_lipschitz,
                                         bilipschitz=bilipschitz,
    ...
    def get_action(self, env: gym.Env, epsilon: float, device: str) -> int:
        ...
        if self.exploration_strategy_name == "epsilon-greedy":
            self.net.eval()
            mean_q_values, entropy_q_values, log_prob_q_values = self.net(state, output_type="mean-var-prob")
            self.net.train()
            ...
            self.net.eval()
            posterior_dist_q_values, log_prob_q_values = self.net(state, output_type="dist")
            mean_q_values = posterior_dist_q_values.maximum_a_posteriori().mean().squeeze(1)  # Useful to track mean
            entropy_q_values = posterior_dist_q_values.maximum_a_posteriori().uncertainty().squeeze(1)  # Useful to track aleatoric
            mean_q_values_samples, var_q_values_samples = posterior_dist_q_values.rsample()  # Sample from epistemic distribution
            self.net.train()
            ...
            mean_q_values = posterior_dist_q_values.maximum_a_posteriori().mean().squeeze(1)  # Useful to track mean
            entropy_q_values = posterior_dist_q_values.maximum_a_posteriori().uncertainty().squeeze(1)  # Useful to track aleatoric
            mean_q_values_samples = posterior_dist_q_values.maximum_a_posteriori().rsample()  # Sample from aleatoric distribution
            self.net.train()
        ...
        return action, mean_q_values[:, action], entropy_q_values[:, action], log_prob_q_values[:, action]
    ...
    def play_step(
        ...
        Returns:
            reward, done, epistemic uncertainty, aleatoric uncertainty
        """

        action, mean_q_value, var_q_value, log_prob_q_value = self.get_action(env, epsilon, device)
        ...
        if done:
            self.reset(env)
        return reward, done, - log_prob_q_value, var_q_value
    ...
    def compute_prediction_stastistics(self, batch: Tuple[Tensor, Tensor]):
        ...
        with torch.no_grad():
            mean_state_action_values, var_state_action_values, log_prob_state_action_values = self.net(states)
            ...
            log_prob_state_action_values = log_prob_state_action_values.gather(1, actions.unsqueeze(-1)).squeeze(-1)

        return mean_state_action_values.mean(), var_state_action_values.mean(), - log_prob_state_action_values.mean()
    ...
    def mse_loss(self, batch: Tuple[Tensor, Tensor]) -> Tensor:
        ...
        with torch.no_grad():
            mean_next_state_values, var_next_state_values, log_prob_next_state_values = self.target_net(next_states)
