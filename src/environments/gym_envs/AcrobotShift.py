...
class AcrobotShift(core.Env):
    ...
    **REFERENCE:**
    ...
    def __init__(self, state_shift=0., action_shift=0., transition_shift=0., reward_shift=0., init_shift=0., when_anomaly_starts=None):
        ...
        self.LINK_LENGTH_1 = (1. + np.random.uniform(low=-self.transition_shift, high=self.transition_shift)) * 1.
        self.LINK_LENGTH_2 = (1. + np.random.uniform(low=-self.transition_shift, high=self.transition_shift)) * 1.
        self.LINK_MASS_1 = (1. + np.random.uniform(low=-self.transition_shift, high=self.transition_shift)) * 1. # TODO check the "self" defintions
        self.LINK_MASS_2 = (1. + np.random.uniform(low=-self.transition_shift, high=self.transition_shift)) * 1.
        self.LINK_COM_POS_1 = (1. + np.random.uniform(low=-self.transition_shift, high=self.transition_shift)) * 0.5
        self.LINK_COM_POS_2 = (1. + np.random.uniform(low=-self.transition_shift, high=self.transition_shift)) * 0.5
        self.LINK_MOI = (1. + np.random.uniform(low=-self.transition_shift, high=self.transition_shift)) * 1.
    ...
    def reset(self):
        ...
        # Init
        init_shift = (1. + np.abs(self.init_shift * np.random.randn())) * 0.1
        self.state = self.np_random.uniform(low=-init_shift, high=init_shift, size=(4,))
        self.random_steps = []
        # TODO: Remove ?
        self.selected_sensors = np.zeros(self.observation_space.shape[0])
        self.selected_sensors[: int(len(self.selected_sensors) / 3)] = 1
    ...
    def step(self, a):
        ...
        # torque = (1 + self.action_shift * np.random.randn()) * self.AVAIL_TORQUE[a]
